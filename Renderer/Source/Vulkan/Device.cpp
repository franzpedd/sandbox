...
namespace Cosmos::Vulkan
{
	Device::Device(std::shared_ptr<Instance> instance)
		: mInstance(instance)
	{
		LOG_ASSERT(mSurface != VK_NULL_HANDLE, "Create the surface object before initializing the Vulkan Renderer");
	...
	Device::QueueFamilyIndices Device::FindQueueFamilies(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface)
	{
		...
		if (!indices.compute.has_value()) LOG_TO_TERMINAL("[Warn]: A compute queue was not found");
		return indices;
	}

	VkSampleCountFlagBits Device::GetMaxUsableSamples()
	{
		LOG_TO_TERMINAL("[Todo]: Move MSAA samples to settings.ini");
	...
	VkResult Device::CreateBuffer(VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkDeviceSize size, VkBuffer* buffer, VmaAllocation* memory, void* data)
	{
		...
		LOG_ASSERT(vmaCreateBuffer(mAllocator, &bufferCI, &allocCI, buffer, memory, &allocInfo) == VK_SUCCESS, "Failed to create buffer");
		...
		{
			void* mapped = nullptr;
			LOG_ASSERT(vmaMapMemory(mAllocator, *memory, &mapped) == VK_SUCCESS, "Failed to map memory");
	...
	VkCommandBuffer Device::CreateCommandBuffer(VkCommandPool cmdPool, VkCommandBufferLevel level, bool begin)
	{
		...
		LOG_ASSERT(vkAllocateCommandBuffers(mDevice, &cmdBufferAllocInfo, &cmdBuffer) == VK_SUCCESS, "Failed to allocate command buffers");
		...
		if (begin)
		{
			...
			cmdBufferBI.flags = 0;
			LOG_ASSERT(vkBeginCommandBuffer(cmdBuffer, &cmdBufferBI) == VK_SUCCESS, "Failed to initialize command buffer");
		}
	...
	{
		LOG_ASSERT(vkEndCommandBuffer(cmdBuffer) == VK_SUCCESS, "Failed to end the recording of the command buffer");
		...
		VkFence fence;
		LOG_ASSERT(vkCreateFence(mDevice, &fenceCI, nullptr, &fence) == VK_SUCCESS, "Failed to create fence for command buffer submission");
		LOG_ASSERT(vkQueueSubmit(queue, 1, &submitInfo, fence) == VK_SUCCESS, "Failed to submit command buffer");
		LOG_ASSERT(vkWaitForFences(mDevice, 1, &fence, VK_TRUE, 100000000000) == VK_SUCCESS, "Failed to wait for fences");
	...
	VkFormat Device::FindSuitableFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
	{
		...
		LOG_ASSERT(resFormat != VK_FORMAT_UNDEFINED, "Failed to find suitable format");
		return resFormat;
	}
	...
	void Device::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, uint32_t arrayLayers, VkSampleCountFlagBits samples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VmaAllocation& allocation, VkImageCreateFlags flags, VmaAllocationCreateFlags vmaFlags)
	{
		...
		// create image
		LOG_ASSERT(vmaCreateImage(mAllocator, &imageCI, &vmaCI, &image, &allocation, nullptr) == VK_SUCCESS, "Failed to create image");
	}
	...
	VkImageView Device::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspect, uint32_t mipLevel, uint32_t layerCount, VkImageViewType viewType)
	{
		...
		LOG_ASSERT(vkCreateImageView(mDevice, &imageViewCI, nullptr, &imageView) == VK_SUCCESS, "Failed to create image view");
	...
	VkSampler Device::CreateSampler(VkFilter min, VkFilter mag, VkSamplerAddressMode u, VkSamplerAddressMode v, VkSamplerAddressMode w, float mipLevels)
	{
		...
		LOG_ASSERT(vkCreateSampler(mDevice, &samplerCI, nullptr, &sampler) == VK_SUCCESS, "Failed to create sampler");
	...
	void Device::SelectPhysicalDevice()
	{
		...
		{
			LOG_TO_TERMINAL("[Warn]: Choosing first discrete GPU available as it's a multiple GPU computer");
		}
	...
	void Device::CreateAllocator()
	{
		...
		LOG_ASSERT(vmaCreateAllocator(&ci, &mAllocator) == VK_SUCCESS, "Failed to create Vulkan Memory Allocator");
